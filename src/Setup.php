...
class Setup extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($envType->is(EnvEnum::SYMFONY_DEV)) {
            $this->emptyStorageDirectories($fs);
        }
    ...
    protected function manalize(SymfonyStyle $output)
    {
        ...
        $stopwatch->start('manalize');

        $process->run(function ($type, $buffer) use ($output) {
            $output->write($buffer);
        });

        $event = $stopwatch->stop('manalize');
        ...
        $output->success(sprintf('Environment successfully created in %dms', $event->getDuration()));
     ...
     * @return
     */
    protected function emptyStorageDirectories(Filesystem $fs)
    {
        $rootStorageDir = is_readable($this->workingDirectory.'/var') ? $this->workingDirectory.'/var' : $this->workingDirectory.'/app';
